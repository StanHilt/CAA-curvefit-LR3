...
peak_list_ctrl <- function(peaks_smooth, range) { 
  ...
  for (i in seq_along(peaks_smooth[range[1]:range[2],])) {
      ...
      conditionalPanel(
        condition = "input.Show_options",
          sliderInput(
            ...
            min = 165, max = 375, value = c(200,350)
          ),
        
        sliderInput(
          "range2", 
          label = h4("Set test peak detection range"),
          min = 0, max = 165, value = c(40,150)
        ),
          textInput(
            "exclude", 
        ...
        mainPanel(
          tabsetPanel(
            ...
            tabPanel("Results", 
                     ...
                     downloadButton("Downloadexcelresults", "Download results as Excel")
...
server <- function(input, output, session) {
  ...
  test_samples <- reactive({
    n <- as.character(if (input$radio == 1 &
                          ncol(peaks_smooth()) <= 10) {
      NA
    }else{
      if (input$radio == 1) {
  ...
  df <- reactive({
    req(input$file)
    print(input$file)# Ensure input$file is available
  ...
  output$downloadPeaks = downloadHandler(
    filename = function() {
      paste0(gsub(".xlsx", "", input$file$name), "_peaks.png")
    },
    content = function(file) {
      dpi <- 800  # Set the desired DPI
      width_inches <- 6  # Set the desired width in inches
      ...
      withProgress(message = "Downloading", value = 0, {
        print(plot_list_all(peaks_smooth(), df_tidy_smooth(), peaks_list()))
      })
      dev.off()
    }
  )
  ...
  output$downloadavgcurve = downloadHandler(
    filename = function() {
      paste0(gsub(".xlsx", "", input$file$name), "_average_curve.png")
    },
    content = function(file) {
  ...
  output$downloadcurve = downloadHandler(
    filename = function() {
      paste0(gsub(".xlsx", "", input$file$name), "_fitted_curve.png")
    },
    content = function(file) {
      dpi <- 200  # Set the desired DPI
      width_inches <- 8  # Set the desired width in inches
  ...
  output$Downloadresults = downloadHandler(
    filename = function() {
      paste0(gsub(".xlsx", "", input$file$name), "_results.csv")
    },
  ...
  output$Downloadexcelresults = downloadHandler(
    filename = function() {
      paste0(gsub(".xlsx", "", input$file$name), "_results.xlsx")
    },
    content = function(filename) {
